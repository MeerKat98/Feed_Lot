        ...
        private void CmbKraal_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbKraal.Text != "System.Data.DataRowView")
            {
                ...
                try
                {
                    ...
                    con.Close();
                    txtAvgFeed.Text = Convert.ToString(Convert.ToDouble(txtTotalFeed.Text) / Convert.ToInt32(txtCount.Text));
                }
                catch (Exception ex)
                {
                    if (ex.Message == "No data exists for the row/column.")
                        MessageBox.Show("This kraal has not been fed yet!");
                    else if (ex.Message != "No value given for one or more required parameters.")
